...
// as ordinary links in chat markdown. Without this they render like any
// external https link (target="_blank"), so a click opens a second copy of the app in a new
// window/tab instead of switching the Files pane of the window the user is already in.
...
export function handleFilesLinkClick(event: ReactMouseEvent<HTMLAnchorElement>, hash: string): void {
    // The link may sit inside a clickable surface. A link click is never also a click on that
    // surface: a plain click could have the surface close the Files pane again, and a
    // cmd/ctrl-click would switch this tab while the link opens in another.
