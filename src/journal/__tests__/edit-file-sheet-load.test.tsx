...
describe("EditFileSheet — load-then-edit (read_file)", () => {
    ...
    it("the subsequent Save sends the loaded content WITH the auto-filled expected_sha256 (CAS on by default)", async () => {
        ...
        await flush();
        // User tweaks the loaded content, then saves.
        await act(async () => {
    ...
    // --- loaded state must not cross agent/path targets ---
    ...
    it("invalidates the loaded checksum when the user switches boxes (no cross-target CAS)", async () => {
        ...
        // Nor A's loaded contents: the form is back in replace-text mode with an empty whole-file
        // draft, so a Save on B cannot write A's bytes (or an empty file) over B.
        expect(container.querySelector<HTMLTextAreaElement>("#mj-edit-file-content")).toBeNull();
        await act(async () => clickButton(container, "Save edit"));
        await flush();
        expect(editFile).not.toHaveBeenCalled();
        expect(container.textContent).toContain("Enter the exact text to replace.");
    });

    it("retargeting the path after a load drops the loaded contents, so Save can't write A over B", async () => {
        const { client, editFile } = makeClient({
            kind: "loaded",
            path: "/box/a.txt",
        ...
        await act(async () => clickButton(container, "Load current contents"));
        await flush();
        expect(container.querySelector<HTMLTextAreaElement>("#mj-edit-file-content")?.value).toBe("contents of a\n");
    ...
    it("discards a load reply whose path changed while it was in flight (deferred-response race)", async () => {
        ...
        await act(async () => setValue(container, "mj-edit-file-path", "/box/a.txt"));
        await act(async () => clickButton(container, "Load current contents"));
        // User retargets the path BEFORE the (slow) read returns.
        await act(async () => setValue(container, "mj-edit-file-path", "/box/b.txt"));
