...
/*
 * Files-pane write UI. The load-bearing assertions here are the two that
 * protect a live deploy: (1) with `writable:false` the pane renders ZERO write affordances, so the
 * dormant backend shows exactly the read-only browser; (2) no destructive call reaches
 * FilesApi without the user passing through the confirm dialog.
 */
...
describe("write lifecycle under the real app shell", () => {
    ...
    it("mints a FRESH key after a definite refusal (the user is told to change the name)", async () => {
...
// -- Uncertain outcomes pin the payload; the modal contains focus; a BOM survives an edit ---------
...
describe("an UNCERTAIN outcome binds the payload", () => {
    it("locks the upload name while the outcome is unresolved and replays the ORIGINAL payload", async () => {
        // The upload commits but its response is lost. If the user may now edit "Save as", the
        ...
        // The name is no longer the user's to change: the first attempt may have landed.
    ...
    it("refreshes the listing when an unresolved write is dismissed instead of retried", async () => {
        // Cancel is the escape hatch from the bound payload. It must not drop the user back
    ...
    it("still mints a fresh key after a DEFINITE refusal", async () => {
        const move = jest
            .fn()
            .mockRejectedValueOnce(new JournalApiError("exists", 409, "dest-exists"))
            .mockResolvedValue({ from: `${DIR}/notes.md`, to: `${DIR}/n3.md`, dryRun: false });
        ...
        // reconcile and the user is being ASKED to pick another name.
...
describe("the modal contains focus while a request is in flight", () => {
...
describe("a UTF-8 BOM survives an edit", () => {
...
// -- classify the outcome by ORIGIN, not by HTTP status ----------------
...
describe("a LOCAL refusal is definite — nothing was sent", () => {
    it("leaves a stale-edit draft editable instead of pinning it as an unresolved write", async () => {
        ...
        // looks exactly like a lost response, which would lock the editor and tell the user
...
// -- a gateway 5xx is not a verdict, and a pin is only a replay while the key lives -----

describe("a gateway failure is an unresolved outcome", () => {
    ...
    it("treats 507 as unknown too — a bare status does not authenticate its origin", async () => {
...
describe("a pinned replay expires before the server forgets the key", () => {
...
// -- the deadline is only honest if it is anchored to the send, and an expiry must not
//    quietly throw away the rest of the user's selection ------------------------------------

describe("the replay deadline is anchored to the first send", () => {
    beforeEach(() => jest.useFakeTimers({ doNotFake: ["queueMicrotask"] }));
    afterEach(() => jest.useRealTimers());
...
describe("an expiry keeps the rest of the upload queue", () => {
    beforeEach(() => jest.useFakeTimers({ doNotFake: ["queueMicrotask"] }));
    afterEach(() => jest.useRealTimers());

    it("carries the unprocessed files forward instead of dropping them silently", async () => {
        // The user picked three files and cancelled nothing. Reconciling the whole PendingWrite
    ...
    it("an explicit cancel still abandons the whole selection — that IS the user saying stop", async () => {
        const upload = jest.fn().mockRejectedValue(new JournalApiError("timed out", 0, "timeout"));
        const api = mockApi({ upload: upload as unknown as FilesApiLike["upload"] });
        const pane = await mountPane(api);
        await pickUpload(
...
// -- the carried queue must not punch through the reconciliation barrier, and a wall
//    clock is not a trustworthy deadline on its own ----------------------------------------------

describe("a carried upload queue stays behind the reconciliation barrier", () => {
    beforeEach(() => jest.useFakeTimers({ doNotFake: ["queueMicrotask"] }));
    afterEach(() => jest.useRealTimers());
    ...
    it("never re-offers it when the refreshed directory comes back read-only", async () => {
        // The capability can be revoked underneath a user mid-selection. Re-offering the
...
describe("the replay deadline survives a wall-clock rollback", () => {
    beforeEach(() => jest.useFakeTimers({ doNotFake: ["queueMicrotask"] }));
    afterEach(() => jest.useRealTimers());

    it("expires on elapsed time even when the system clock is set backwards", async () => {
        // NTP correction, a laptop waking up, or a user fixing their clock. Measured on the
...
// -- a SUCCESSFUL queue step goes behind the same barrier, and the capability is
//    re-checked at the moment of acting, not only where the affordance was drawn ----------------

describe("a successful upload releases the next file through the barrier", () => {
...
describe("the write capability is re-checked where the write is SENT", () => {
    ...
    it("refuses to send when the directory stopped being writable while the dialog sat open", async () => {
        ...
        // affordance RENDERS does not gate the action — this is the check at the send.
        const api = mockApi();
        container = document.createElement("div");
        document.body.append(container);
        await act(async () => {
...
describe("navigating away drops a parked upload queue", () => {
    let writes: ReturnType<typeof useFileWrites> | undefined;
...
describe("a read-only answer ENDS a parked queue", () => {
    let writes: ReturnType<typeof useFileWrites> | undefined;
    ...
    it("does not resurrect it when the same directory becomes writable again later", async () => {
        ...
        // straight into the release and re-offered a selection the user was told was dropped.
        const api = mockApi({ upload: jest.fn().mockResolvedValue({ path: `${DIR}/a`, bytes: 1, dryRun: false }) });
        container = document.createElement("div");
        document.body.append(container);
        await act(async () => {
...
describe("a failed re-read is not an answer", () => {
    let writes: ReturnType<typeof useFileWrites> | undefined;
    ...
    it("keeps a parked queue across a transient listing failure and releases it on the retry", async () => {
        ...
        // user's selection — the one outcome the expiry notice explicitly promises against,
