...
describe("client.applyFilesDeepLink", () => {
    ...
    it("ignores the link when this deploy has not enabled Files (no config files_root)", () => {
        const abs = `${DIR}/notes.md`;
        window.location.hash = `#files=${encodeURIComponent(abs)}`;
        const client = signedInClient();
        (client as unknown as { state: ClientState }).state = { ...client.getSnapshot(), config: {} };
        client.applyFilesDeepLink();
        expect(client.getSnapshot().filesView).toBeUndefined();
...
describe("FilesPane deep-link auto-preview", () => {
    ...
    it("does not consume the prior listing when a deep link arrives at an already-loaded pane (key-generation guard)", async () => {
        // Mount plain at DIR: the first listing contains ONLY old.md (no notes.md). Then a deep
        // link for DIR/notes.md arrives via rerender. The fresh (nonce-keyed) listDir returns a
        // listing WITH notes.md. If the pane consumed the stale first listing (the one-commit
        // window), it would find no notes.md and give up; the key guard makes it wait for the
        // fresh response and select notes.md.
        ...
        await rerenderPane({ open: true, path: DIR, targetFile: `${DIR}/notes.md`, targetToken: 1 });
        const selectedRow = pane.querySelector(".mj_FilesRow_selected");
        expect(selectedRow).not.toBeNull();
        expect(selectedRow?.textContent).toContain("notes.md");
    });
    ...
    it("does not auto-select from a STALE listing when the target directory differs (wrong-file guard)", async () => {
        ...
        // own listing and never select notes.md out of OTHER_DIR's payload.
        const OTHER = "/home/user/other";
        const otherListing: FileListing = {
            path: OTHER,
            root: OTHER,
            parent: null,
            entries: [{ name: "notes.md", kind: "file", size: 9, mtime: 1, mime: "text/markdown" }],
            truncated: false,
            writable: false,
        };
        const api = mockApi({
        ...
        const selectedRow = pane.querySelector(".mj_FilesRow_selected");
        // It navigated to DIR and selected DIR's notes.md — not OTHER's.
        expect(selectedRow).not.toBeNull();
