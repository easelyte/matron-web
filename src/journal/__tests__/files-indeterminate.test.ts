 ...
 * a user "Nothing was changed" in the second case is not a vague message, it is a false one —
...
describe("DELETE carries an Idempotency-Key", () => {
    it("puts the key on the wire, so a lost response replays instead of deleting again", async () => {
        // The journal's delete route runs under `withIdempotency`. Without a key every retry would be a
        // fresh execution — and if another actor had recreated the path in the gap, it would delete
        // the REPLACEMENT.
        const fetchMock = jest
            .fn()
            .mockResolvedValue(
