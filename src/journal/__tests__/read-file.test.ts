...
describe("classifyReadFileReply", () => {
    ...
    it("explains an older bridge without the read_file RPC instead of echoing the raw code", () => {
        const outcome = classifyReadFileReply({ ok: false, origin: "agent", code: "unknown_method" });
        expect(outcome.kind).toBe("error");
        expect(outcome.kind === "error" && outcome.message).toMatch(/doesn't support loading files/);
    });
