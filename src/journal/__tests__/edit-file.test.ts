...
describe("classifyEditFileReply", () => {
    ...
    it("explains an older bridge without the edit_file RPC instead of echoing the raw code", () => {
        const outcome = classifyEditFileReply({ ok: false, origin: "agent", code: "unknown_method" });
        expect(outcome.kind).toBe("error");
        expect(outcome.kind === "error" && outcome.message).toMatch(/doesn't support file editing/);
    });
