 ...
 * error-code mapping — the safety-critical CAS + path-safety surface — are unit
...
export function classifyEditFileReply(reply: RpcReply): EditFileOutcome {
    ...
    if (reply.origin !== "agent") {
        ...
        // retry (no bridge dedupe). See the "uncertain" outcome.
    ...
    switch (reply.code) {
        case "unknown_method": // a bridge that predates the edit_file RPC
            return {
                kind: "error",
                message: "This box's bridge doesn't support file editing yet. Update the bridge on that box.",
            };
        case "stale":
            return { kind: "stale" };
        case "not_found":
