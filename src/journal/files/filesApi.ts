...
/*
 * Matron File Explorer — client-side file API (reads + writes).
 *
 ...
 *
 * The write half (upload / mkdir / move / write / delete). Those methods exist on the
...
import { DOWNLOAD_TIMEOUT_MS, DOWNLOAD_URL_TTL_MS, FETCH_TIMEOUT_MS, WRITE_TIMEOUT_MS } from "./limits";
...
export interface FileListing {
    path: string;
    /**
     * The configured read-root that CONTAINS `path` (realpath-canonical), per the breadcrumb-jail contract.
    ...
    truncated: boolean;
    /**
     * The server will accept writes INTO this directory (it is inside a write-root, writes
     * are enabled, and it is not in dry-run). Absent on a read-only server ⇒ false ⇒ the pane renders
...
/**
 * The surface FilesPane depends on. The real FilesApi and test mocks both implement it, so
...
export interface FilesApiLike {
    ...
    /** Raw bytes for the pdf.js/canvas renderer. pdf.js takes ownership of the buffer. */
     ...
     * an agent rewriting the file at the same path busts the cache. Revoked on dispose.
     */
    contentUrl(
        path: string,
    ...
    // ── Writes. Only reachable from UI that the server said is `writable`. ──────────────
    /**
...
/**
 * Reduce a user-typed or OS-supplied name to a single safe path component. This is UX, NOT
 ...
 * instead and tell the user to go look.
 */
...
export class FilesApi implements FilesApiLike {
    ...
    // Current (newest-mtime) cache entry per (disposition, path): lets us revoke the PRIOR mtime
    // generation when a newer one is cached, so a long session doesn't retain every previewed revision.
    private readonly generations = new Map<string, { key: string; mtime: number }>();
    ...
    public async listDir(path: string, all = false, signal?: AbortSignal): Promise<FileListing> {
        ...
        return {
            path: resolvedPath,
            // Older server without the `root` field: fall back to the path itself so the breadcrumb
            ...
            // Read-only servers omit the flag entirely — absent ⇒ read-only, never assumed writable.
    ...
    public contentUrl(
        ...
        const request = (async (): Promise<string> => {
            try {
                // The shared request is deliberately NOT tied to `opts.signal`: a sibling
                ...
                // URL so repeatedly previewing a rewritten file doesn't accumulate every revision.
                // Completions can land out of order (two generations in flight at once), so only a
                // NEWER mtime supersedes: an older one finishing late must not revoke the URL the
                // current preview is showing. It stays cached until dispose.
                const genKey = `${disposition}:${path}`;
                const prev = this.generations.get(genKey);
                if (!prev || opts.mtime >= prev.mtime) {
                    if (prev && prev.key !== cacheKey) {
                        const prevUrl = this.urls.get(prev.key);
                        if (prevUrl) {
                            URL.revokeObjectURL(prevUrl);
                            this.urls.delete(prev.key);
                        }
                    }
                    this.generations.set(genKey, { key: cacheKey, mtime: opts.mtime });
                }
    ...
    public async download(path: string, filename: string, signal?: AbortSignal): Promise<void> {
        ...
        // the (transfer-sized) timeout via request() so a stalled attachment stream can't hang forever.
        const blob = await this.request(this.contentPath(path, "attachment"), signal, (response) => response.blob(), {
            timeoutMs: DOWNLOAD_TIMEOUT_MS,
        });
        // Completed after sign-out → mint nothing; never save old-session bytes to disk.
    ...
    public async deleteEntry(
        ...
        // DELETE is the most destructive write: without a key, a retry whose response was lost would
        // re-execute instead of replaying — and if another actor had recreated the path in the gap,
        // it would delete the REPLACEMENT. The journal's delete route runs under `withIdempotency`
        // and reads this header.
        //
    ...
    // `consume` INSIDE this timeout scope: a server that returns 2xx headers then stalls the body
...
// Uniform, reason-agnostic user-facing copy. The server deliberately does not leak WHY a path
// was denied (sensitive vs outside-scope both map to 403), so the client mustn't either.
/**
 * True when a /files response says this account may not use the file routes at all (403 with the
 * journal's `forbidden` code), as opposed to a per-path refusal (`denied`). The client then hides
 * the Files surface for the session rather than rendering an error pane.
 */
export function isFilesForbidden(status: number | undefined, code: string | undefined): boolean {
...
export function messageForFileStatus(status: number, code?: string): string {
    ...
    switch (status) {
        ...
        case 507:
            ...
            // a user gets told a delete did not happen when it may well have.
            //
