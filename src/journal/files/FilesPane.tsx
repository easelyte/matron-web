...
/*
 * Files pane — the user-visible "consult files" surface (Matron File Explorer).
 ...
 *
 * Writes (new folder / upload / rename / delete / edit), every one of them gated on
 ...
 * deploy is byte-for-byte the read-only pane.
 */
...
import { type ClientState, filesRootFromConfig } from "../types";
import { FileWriteDialog } from "./FileWriteDialog";
import { type FileEntry, type FileListing, type FilesApiLike, isFilesForbidden } from "./filesApi";
...
interface Selected {
    ...
    // meta — an agent may have rewritten the file since it was last opened.
    at: number;
}
...
interface RowData {
     ...
     * directory is writable. False (or a read-only server that omits the flag) renders no write
...
export function FilesPane({ client, state }: { client: MatronJournalClient; state: ClientState }): React.ReactElement {
    ...
    const [dir, setDir] = useState(
        // With no remembered path, open at the deploy's configured Files root (config `files_root`,
        // which the server must have within MATRON_FILE_READ_ROOTS).
        () => state.filesView?.path ?? filesRootFromConfig(state.config) ?? "/",
    );
    ...
    // A 403 `forbidden` (as opposed to a per-path `denied`) means this ACCOUNT may not use the
    // file routes at all. Degrade to the hidden state for the session instead of showing an error
    // pane the user can do nothing about.
    useEffect(() => {
        if (listing.status === "error" && isFilesForbidden(listing.errorStatus, listing.errorCode)) {
            client.markFilesUnavailable();
        }
    }, [client, listing.status, listing.errorStatus, listing.errorCode]);
    ...
    // outcome is unknown, or an uncertain write the user backed out of). It holds because
    ...
    // against the stale directory state the user was just told to go and check. If the re-read
    ...
    // Breadcrumb spans root → path ONLY (never above the read-root jail). Falls back to the
            ...
            <div className="mj_FilesPane_body" inert={writes.state ? true : undefined}>
                <div className="mj_FilesPane_nav">
                    ...
                    {/* Write toolbar — mounted ONLY when the server reported this directory
                        ...
                        unchanged read-only browser, down to the DOM. */}
                    {writable ? (
