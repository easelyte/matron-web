...
/*
 * The one modal every Files-pane write passes through. It reuses the shipped
 * upload-confirm shell (`mj_UploadConfirm*` — scrim, card, header, footer) so a write confirm is
 * visually the same object the user already knows from sending an attachment, and so the app's
 ...
 * collects the user's input (new name / edited text).
 */
...
function EditBody({
    ...
    // Load the CURRENT bytes at open time — the user edits what is on disk right now, not a
...
export function FileWriteDialog({
    ...
    // Escape closes — but only while the user still owns the decision (canDismiss). A request
