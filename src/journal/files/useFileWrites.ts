...
/*
 * React binding for the Files write confirm machine. Owns the phase state,
 ...
 * toggling hidden files, a retry) would resurrect a selection the user had been told was gone.
 *
...
export interface DirectoryReadiness {
     ...
     * with a Retry on screen — a dropped socket must not cost the user their selection, and the
...
export function useFileWrites(
    ...
    const begin = useCallback((pending: PendingWrite) => {
        setNotice(undefined);
        // A new write the user started themselves supersedes anything parked.
    ...
    // (FilesPane.onWritten). The user decides from what is actually there rather than
    ...
    const reconcile = useCallback(
        (current: WriteState, expired: boolean) => {
            ...
            // An EXPIRY is not a decision the user made — it is a deadline passing while they
            ...
            // so no further mutation can be aimed at directory state the user was just told to
            ...
            //
            // An explicit cancel is the opposite — the user asking to stop — and still
    ...
    // that comes back without the capability, simply never releases it — the user is left in
    // the read-only view the server actually authorized, which is the safe way round. Navigating
    // elsewhere drops it: that is the user moving on, and resuming into another directory
    ...
    useEffect(() => {
        ...
        // directory later would resurrect a dialog the user had reason to believe was gone.
        if (next.kind !== "upload" || next.dir !== dirPath) {
            held.current = undefined;
            return;
        }
        // Still out, or it failed and the user has a Retry in front of them: the barrier stays
    ...
    const submit = useCallback(
        (input: WriteInput) => {
            ...
            if (!dirWritable) {
                ...
                // the action. Backstop to the barrier above, not a replacement for it.
                setNotice(
            ...
            void (async () => {
                try {
                    ...
                    if (current.pending.kind === "delete" && outcomeIsUnknown(error)) {
                        ...
                        // intermediary that already let the write through. Making the user's
