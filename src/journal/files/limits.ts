...
// inline-render limit and the transport limits are not scattered.
 ...
 * mid-stream would leave the user retrying forever. Still bounded, so a stalled socket fails
...
/**
 * Timeout for a Download (attachment) transfer. The download fallback exists precisely for files
 * too large to preview, so it gets a transfer-sized deadline rather than the 30 s read one: a
 * multi-megabyte file over a slow phone link legitimately takes longer. Still bounded, so a
 * stalled socket fails visibly rather than leaving the button spinning.
 */
