...
export type ReadFileOutcome =
     ...
     * so the user can still author content manually.
     */
    | { kind: "not-found" }
...
export function classifyReadFileReply(reply: RpcReply): ReadFileOutcome {
    ...
    switch (reply.code) {
        case "unknown_method": // a bridge that predates the read_file RPC
            return {
                kind: "error",
                message: "This box's bridge doesn't support loading files yet. Update the bridge on that box.",
            };
