...
import { filesRootFromConfig } from "../../../src/journal/types";
...
// breadcrumbs are built from the read-root DOWN to path — NEVER above the jail.
describe("breadcrumb (read-root jail)", () => {
    ...
    it("at the root itself → a single root crumb", () => {
        expect(breadcrumb(root, root)).toEqual([{ label: "project", path: root }]);
    });

    it("path not under root → only the root crumb (never fabricates ancestors)", () => {
        expect(breadcrumb(root, "/etc/passwd")).toEqual([{ label: "project", path: root }]);
    });
...
// ── FilesApi: transport parsing + lifecycle ────────────────────────────────────────
...
describe("FilesApi", () => {
    ...
    it("parses a directory listing incl. the `root` field and drops malformed entries", async () => {
    ...
    it("maps a 403 to a typed JournalApiError", async () => {
        globalThis.fetch = jest
            .fn()
            .mockResolvedValue(fakeResponse({ status: 403, body: { error: "denied" } })) as unknown as typeof fetch;
        const api = new FilesApi(SERVER, "tok");
        await expect(api.listDir("/etc")).rejects.toMatchObject({ status: 403, code: "denied" });
    });

    // content cache keyed by (disposition, mtime, path) — a changed mtime busts it.
    ...
    // a superseded mtime generation is revoked so a long session doesn't retain every revision.
    ...
    it("an OLDER generation finishing late does not revoke the newer URL being shown", async () => {
        const releases: Array<() => void> = [];
        globalThis.fetch = jest.fn(
            () =>
                new Promise<Response>((resolve) => {
                    releases.push(() => resolve(fakeResponse({ status: 200 })));
                }),
        ) as unknown as typeof fetch;
        const api = new FilesApi(SERVER, "tok");
        const older = api.contentUrl("/root/f.png", { mtime: 1 });
        const newer = api.contentUrl("/root/f.png", { mtime: 2 });
        releases[1](); // mtime 2 lands first and is what the preview shows
        const g2 = await newer;
        releases[0](); // then the stale mtime 1 request completes
        const g1 = await older;
        expect(revoked).not.toContain(g2);
        expect(await api.contentUrl("/root/f.png", { mtime: 2 })).toBe(g2); // still cached, still live
        // A later, newer generation still supersedes the current one.
        globalThis.fetch = jest.fn().mockResolvedValue(fakeResponse({ status: 200 })) as unknown as typeof fetch;
        await api.contentUrl("/root/f.png", { mtime: 3 });
        expect(revoked).toContain(g2);
        expect(g1).not.toBe(g2);
    });

    // the shared (deduped) content request is NOT bound to one view's AbortSignal — a sibling
    ...
    // the body read is bounded by the timeout, not just the header phase — a 2xx-then-stalled
    ...
    // Download is the fallback for files too large to preview, so it gets a transfer-sized deadline:
    // a slow link must not be cut off at the 30 s read deadline, but a stalled one still fails.
    it("gives a Download the longer transfer deadline, still bounded", async () => {
        jest.useFakeTimers();
        globalThis.fetch = jest.fn((_url: unknown, opts?: { signal?: AbortSignal }) => {
            const signal = opts?.signal;
        ...
        const api = new FilesApi(SERVER, "tok");
        let settled = false;
        const pending = api.download("/root/big.mp4", "big.mp4").finally(() => {
            settled = true;
        });
        ...
        await Promise.resolve();
        await Promise.resolve();
        expect(settled).toBe(false); // past the read deadline, still transferring
        jest.advanceTimersByTime(DOWNLOAD_TIMEOUT_MS);
        await expect(pending).rejects.toMatchObject({ code: "timeout" });
    });

    // dispose aborts an in-flight request (abort-on-teardown).
    ...
    // a content fetch that RESOLVES after sign-out mints a URL that is dropped AND revoked.
    ...
    // download rejects on a denial so the caller can surface it (not an unhandled rejection).
    ...
    // downloads are NOT session-cached and their URL is revoked on a TTL.
    ...
    // a download whose bytes arrive after sign-out is dropped — old-session bytes never saved.
...
describe("FilesApi writes", () => {
    ...
    it("gives 409 and 507 their own user copy (never the generic fallback)", () => {
    ...
    // a write response that does not match the contract is NOT a success. The client must
...
// -- Confirm machine: the safety rules, independent of any dialog ------------------------
describe("write confirm machine", () => {
    ...
    it("a failure returns to confirming WITH the error (never silently closes)", () => {
        ...
        // Definite refusal: the caller mints a fresh key, because the user is about to change
...
describe("filesRootFromConfig (the per-deploy Files switch)", () => {
    it("enables Files only for an absolute files_root", () => {
        expect(filesRootFromConfig({ files_root: "/home/user/project" })).toBe("/home/user/project");
        expect(filesRootFromConfig({})).toBeUndefined();
        expect(filesRootFromConfig(undefined)).toBeUndefined();
        expect(filesRootFromConfig({ files_root: "relative/dir" })).toBeUndefined();
        expect(filesRootFromConfig({ files_root: "" })).toBeUndefined();
    });
});
