...
describe("subchat derivations", () => {
    ...
    describe("buildSidebarIndex placement resolution (#536)", () => {
        ...
        it("#541: collapsing a parent hides its running child (nested → hidden), consistently", () => {
        ...
        it("#541: hasSubagentChildRows tracks the host relationship regardless of collapse", () => {
            const P = convo("P", { session_state: "running" });
            const C = convo("C", { parent_convo_id: "P", session_state: "running" });
            const convos = [P, C];
            const empty = new Set<string>();
        ...
        it("#541: collapsing a NON-host parent does nothing (orphan/archived children unaffected)", () => {
