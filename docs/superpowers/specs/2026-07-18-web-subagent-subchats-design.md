...
- **Fail visible on a blocked send (P3 — never silently delete user data).** When an egress guard blocks a child-targeted outbox record (text **or** attachment — `PendingMessage.attachState`/`errorKind`/`canRetry` apply to both), it must **not** silently drop it (Codex R3-B3 = data loss). Instead: skip transmission, **leave the record in the outbox** (not deleted), and mark it a **permanent error** (`attachState:"error"`, `canRetry:false`, message "Can't send to a read-only subagent transcript"). It renders as a dismissable errored row in the child's read-only pending area; reconnect replay re-encounters it but the guard makes re-marking idempotent (never a send). **Dismissal must exist for text rows too, not just attachments** (Codex R4-M2): today only `dismissAttachment` (`client.ts:807`) exists — T5 wires a per-row dismiss for errored text pending messages as well, so the user always has an explicit (non-silent) exit.
...
7. **Fail-visible blocked send (Codex R3-B3 / R4-M2 — P3)** — a child-targeted outbox record (text **or** attachment) is not deleted: it stays in the outbox, is marked `attachState:"error"`, `canRetry:false`, with the read-only message, and remains **dismissable including for text rows** (not only attachments).
