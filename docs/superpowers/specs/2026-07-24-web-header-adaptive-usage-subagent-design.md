...
- `worstLimit(limits)` helper (pure, in `status.ts`): returns the limit with the highest **normalized** `percent` (ties → first) — drives the collapsed mini pill. **Consistency rule (round-2 Codex M1):** normalization is uniform — a non-finite `percent` normalizes to `0` **everywhere**, including `worstLimit` ranking. So `worstLimit` returns an entry (a `0%` one, worst-case) whenever the trimmed limits array is non-empty; there is **no** "all-non-finite → no mini" special case (that created a state where expanded rows rendered `0%` but no trigger could reach them). The empty-usage path fires **only** when the trimmed limits array is length 0 (nothing to show at all) — matching the existing `.mj_HeaderCluster_empty` behavior. Unit-testable.
...
- **`.mj_UsageCluster` / `.mj_UsageBars` / `.mj_UsageRow`** (#501): refit to the compact multi-metric layout — a dense grid `[label] [track] [pct]`, micro type, `--cpd-*` spacing. Percent cell right-aligned, `--cpd-color-text-secondary`, color-coded to match fill. Keep `--cpd-color-usage-{low,medium,high}` + `--cpd-color-bg-usage-track`. Verify light + dark.
  ...
  - **720px override reconciliation + popover-scope fix (m1 + round-2 M2).** The existing `@container mj-room (max-width: 720px)` rule (`journal.pcss:321-324`) collapses `.mj_UsageRow` to a single column, sized for today's 2-cell `[label][track]` row. Under the 3-cell restyle, update it to drop the **label** cell (least information — track+pct carry the signal) to `[track] [pct]`, single-line, in the 460–720 band. **Critical scoping:** because there is **no `createPortal`** in `components.tsx`, the `.mj_UsagePopover` renders as a DOM descendant of `.mx_RoomView_body` and stays inside the `mj-room` container-query scope; since `usageCollapsed` only fires when that container is `<700` (always `<720`), an un-scoped `.mj_UsageRow` override would drop the label **inside the popover too**, defeating the expansion (round-2 M2). The label-drop override must therefore be **selector-scoped to the inline cluster only** — `@container mj-room (max-width: 720px) { .mj_UsageCluster .mj_UsageRow { … } }` — and the popover rows (`.mj_UsagePopover .mj_UsageRow`, a different ancestor class) keep the full `[label][track][pct]`. The inline cluster lives under `.mj_HeaderCluster.mj_UsageCluster`; the popover under `.mj_HeaderMenu.mj_UsagePopover` — distinct scopes, so the scoping is clean. Verify in the visual gate that the popover shows labels.
...
- **`.mj_UsagePopover` / `.mj_TitlePopover`** (#500): `.mj_HeaderMenu` variants — position under their trigger (`top: calc(100% + space-2x)`; `.mj_UsagePopover` right-anchored `right: 0` under the mini-usage pill in the right cluster; `.mj_TitlePopover` left-anchored `left: 0` under the mini-title in the center — clamped to the viewport via the inherited `width: min(280px, calc(100vw - 32px))`), inherit the card treatment.
