...
**Bound the decode work only — NOT the link render.** The `MAX_VIEWER_TOKEN_LEN` guard lives ONLY inside `decodeViewerExp`; do **not** add it to `parseDiffPayload`'s existing https block. An earlier revision gated the https block on length too, but that regressed a valid-but-oversized `viewer_url` (e.g. a token whose JSON-escaped `path`+`workdir` push it past the ceiling) from "live link, no expiry" to "no link at all" (Codex phase-1 review). The bound's purpose is to skip the expensive `atob`/`JSON.parse` on a pathological token, not to withhold a link the viewer would still honor. The pre-existing `new URL(payload.viewer_url)` at `components.tsx:1305` is left unchanged: the `viewer_url` string is already received and `JSON.parse`d by the journal client before `parseDiffPayload` runs, so that parse is not a fresh unbounded-allocation DoS surface. Net: an oversized token ⇒ `viewerUrl` still set (live link), `viewerUrlExp` `undefined` (expiry skipped) ⇒ degrades to today's behavior.
...
**Accepted limitation — mid-session wall-clock correction is not re-detected.** The timer is armed against `Date.now()` at mount, and `nowSec` (React state) only updates when the timer fires. A mid-session OS wall-clock correction (e.g. NTP resync of a badly-drifted clock) is therefore not re-detected, in either direction:
- **Forward correction** (clock was slow → jumps ahead): the armed relative timeout does not reschedule, so a still-live card can keep a link slightly past real expiry until the originally-armed timer fires — at which point it self-heals (the timer was already scheduled).
- **Backward correction** (clock was >30s fast → a valid link was wrongly classified expired, then jumps back): the effect early-returns once `expired` is true, so no timer is armed and `nowSec` never re-reads; the card stays wrongly-expired (withholding a link the viewer would accept) until it remounts/reloads. This does **not** self-heal on a plain re-render, because `nowSec` is timer-updated state, not recomputed from `Date.now()` each render.

Both are conscious accepts, not fix targets. They require a grossly-wrong OS clock (tens of seconds+), which on any real machine breaks TLS and much else long before a diff-card link matters; the viewer remains authoritative (no false capability — a stale-live link still errors on click); and remount/reload recovers. Defending them would need a Page-Visibility / periodic-resync listener that keeps re-reading the clock for every mounted (incl. expired) card — complexity disproportionate to a vanishingly rare cosmetic gap (per `feedback_right_size_effort_to_gain`). The mandatory fake-timer tests advance a monotonic fixed clock and deliberately do not exercise clock reversal.
