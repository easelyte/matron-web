...
```
Because the live client sits behind login, build a **static harness HTML in a scratch dir OUTSIDE `webapp/`** (e.g. `/tmp/reskin-harness/`, importing the built CSS — round-1 plan M2 / final-review M1: `webapp/` is the rimraf'd build output, so a harness there is erased by the build or shipped publicly) that renders representative markup for each in-scope surface under both `:root` and `[data-theme="dark"]`. Screenshot + Read each and compare against the anchor render before deploying. (Canonical location is the plan's T-5.2; this spec defers to it.)
...
7. Dark on-accent text = `#0b201c` on the bright-emphasis surfaces (send circle, upload-send) which pass WCAG AA; **`.mx_Login_submit` is decoupled to light text** (its deep-accent `#0f766e` bg fails at 3.10:1 with `#0b201c` — round-4 B3); `.mj_UnreadBadge` is decoupled to `--cpd-color-text-on-badge` and passes AA on its neutral bg in **both** themes (light bare pass ≥ 4.5:1 compute-verified). All on-accent consumers compute-verified AA, both themes. **Threshold note:** button/action labels (send circle, upload-send — 16px/600) are WCAG **large text** → the 3:1 AA threshold applies, not 4.5:1 (light upload-send `#fff` on `#0d9488` = 3.74:1 passes as large text). **Message timestamps are small (10px) → 4.5:1 applies**, so they use `--cpd-color-text-secondary` (~5.7:1), NOT tertiary (final-review round-2 B3).
