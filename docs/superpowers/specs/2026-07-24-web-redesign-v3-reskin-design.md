...
**Ship:** the implementation `SHA` is already committed to `vps-redesign-v3-reskin` (invariant 0) → `/ship-slim` (Codex adversarial on the diff) → PR to easelyte/matron-web → merge. **Provenance (round-5 Maj1):** if the merge squashes/rebases (new commit id), the `manifest.json` records BOTH the built/deployed `SHA` and the resulting main commit, and the merge is verified with `git merge-base --is-ancestor "$SHA" origin/main` OR the squash-commit message cites `$SHA` — so evidence↔deployed↔merged stay provably linked even when the id changes. Update `docs/design/primitives-v2.md` resume notes / `docs/styling-primitives.md` if the reskin changed any documented primitive.
