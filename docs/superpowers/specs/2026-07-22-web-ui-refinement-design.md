...
3. **Polish, not features.** No new interactions, no new panels, no behavior changes. The LobeHub agent-operator philosophy must **not** pull scope toward features. If a change alters what the app *does* (vs how it *looks*), it is out of scope for #480.
...
```
```ts
// shell bootstrap (src/journal/index.tsx or shell entry) — ~5 lines, theme plumbing, not a feature:
const mq = window.matchMedia("(prefers-color-scheme: dark)");
const applyTheme = () => {
    if (!document.documentElement.hasAttribute("data-theme-user")) // reserved for a future manual toggle
        document.documentElement.setAttribute("data-theme", mq.matches ? "dark" : "light");
};
applyTheme();
mq.addEventListener("change", applyTheme);
```
This is OS-driven dark by default, single-source values, and leaves a `data-theme-user` hook for a future manual toggle (not wired now — a toggle is a feature, out of polish scope). **Alternative if the child wants zero JS:** use a **pure `@media (prefers-color-scheme: dark) :root { … }`** block as the *only* definition (still single-source — one block), dropping the `[data-theme]` selector and the future-toggle hook entirely. Either way the binding rule is **one authoritative copy of the dark values** — never two hand-synced blocks. The child confirms no `data-theme` attribute is currently set by the shell (it is not) before choosing.
- **OPEN QUESTION (operator, §10):** auto-only vs. ship a manual toggle now. Recommendation: auto-only for #480.
...
### 4.3 Ratchet (P17 — required, with an explicit escape; not silently optional)
P17 says a migration that removes a pattern must add a gate against its return, or the pattern creeps back within weeks. The earlier draft left the lint "recommended-not-required," which both round-1 reviewers (Codex Major1) correctly flagged as a P17 miss. **Ratchet ownership follows literal-class ownership (round-2 fix):** each child gates only the literal class it eliminated — so **this (type-scale+spacing) child gates `font-size` + off-grid `spacing`**, and the **dark-theme child gates raw color literals** (§3.2 / §8 item 1), since only it knows which rgb()/hex are permanent theme-invariant exceptions (§2.2 alpha overlays, box-shadows) vs. regressions. A single shared regex/lint rule can cover all three properties, but each child authors + verifies the clause for its own class against its own before/after (avoids a child gating literals it didn't touch and can't classify). **The gate MUST exempt the spec's own sanctioned literals (round-2 Codex B3)** — a naive "only `var()`/`inherit`/`100%`" rule would fail on source this spec explicitly requires keeping: the theme-invariant alpha overlays + box-shadows (§3.2b) and the do-not-touch bubble/composer geometry (`5.5px`/`1px`/`8.5px`/`10px`/`60px`/`-9px`, §5.2). So the ratchet is **property- and exception-scoped**: e.g. the color rule ignores `box-shadow` and the enumerated overlay `background` declarations (via inline `stylelint-disable-next-line`/allowlist comments anchored to those exact lines), and the spacing rule excludes the bubble/composer selectors in §5.2. The allowlist IS the ratchet's record of what's deliberately literal — new literals outside it still fail. A gate that can't pass the spec's own required source is not a ratchet, it's a shipping blocker.

**Required behavior:** add the gate to matron-web's lint step. **Try the cheap path first:** matron-web's `pnpm lint` today is `tsc --noEmit && prettier --check` (no stylelint) — so prefer a **scoped grep-in-CI / custom regex** step (token `var()`s + `inherit`/`100%` + the anchored exemptions above) over adding a stylelint dependency, since the regex satisfies the ratchet without the upstream-alignment tension. **Escape hatch (not a silent skip), only if even the regex step meaningfully conflicts:** file a dedicated `matron-web-css-lint-ratchet` follow-up loop and record the deferral in the ship note. Deferral is a tracked decision, not silence.
...
- **Rebalance the grid.** Reconsider the symmetric `minmax(0,1fr) auto minmax(0,1fr)`. The title stealing width from both sides is the amplifier. Options: `auto auto minmax(0,1fr)` (title flexes, clusters size to content) or give the usage cluster a fixed/min track so it never loses to the title. Child verifies center-title still visually centers acceptably (or accept left/near-center title as the tradeoff — a common chat-header layout).
...
- Manual light/dark toggle **control** (a feature; the `[data-theme]` hook is laid down, the toggle is a follow-up).
...
## 10. Unresolved questions (for operator at review)
1. **Dark theme trigger:** auto (`prefers-color-scheme`) only for #480, with the `[data-theme]` hook laid down for a later toggle? (Recommended.) Or does the operator want the manual toggle control in-scope now (bumps it from polish → small feature)?
2. **Center title:** acceptable to relax strict horizontal centering of the chat title if the grid rebalance (§6.1) needs it, or is dead-center a hard requirement (constrains the banner fix toward option (ii)/min-width over grid reshape)?
3. **Stylelint ratchet:** the spec now *requires* the ratchet with an explicit escape (file a follow-up loop if stylelint is absent + conflicts with upstream-alignment) rather than leaving it optional (§4.3). Confirm this is the right bar — is a required-with-escape ratchet acceptable, or does the operator want it hard-required (block ship until the lint lands) or fully deferred?
