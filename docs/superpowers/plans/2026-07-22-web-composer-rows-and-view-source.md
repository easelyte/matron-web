...
- **Clear-failure resurrect on reload (ship-review r2 B1) → bundled into the idempotency follow-up.** `drafts.clear()` is best-effort like `persist()`: if `localStorage.setItem` throws *during* the post-send clear, the composer clears in-session (correct) but the persisted blob keeps the sent text, so a reload restores it as an apparent unsent draft — a narrow dup-send edge (requires a persisted draft + successful send + a storage failure at the exact clear + a reload + a re-send). This is the localStorage-unreliability wall (any `setItem`/`removeItem` can fail); the dup *consequence* is exactly what the deferred server-side idempotency (`localId` dedup key) neutralizes. Accepted for this client PR, folded into the idempotency follow-up.
