...
- **Post-rebase coordinate re-grounding (round-2 Codex M4 / P35):** all `journal.pcss` line coordinates cited in this plan (`64-67`, `108`, `197-231`, `253`, `321-324`, `353`, `1769-1776`) and `components.tsx` anchors are HEAD-relative and **WILL drift after the T-4.x rebase**. Before implementing CSS edits, and again after the final rebase, `grep -n` each cited selector to reconfirm — treat line numbers as hints, selectors/symbols as the ground truth.
...
**Test-seam / export contract (round-2 Codex M1).** Tests that render/invoke a unit directly require that unit to be importable. `components.tsx` today exports only `MatronApp`. Therefore: `normalizePercent`/`worstLimit` (`status.ts`) get named `export`s (already stated); and `useAdaptiveHeader`, `UsageCluster`, `SubagentStrip` get **named `export`s from `components.tsx`** so their `*-test.ts` files can import them (exporting a symbol is not a "new source file" and doesn't fork Dan's layout — it's additive). `useDismissablePopover`/`HeaderShell` may stay module-private and be exercised through the exported components / full-app render. Each unit's test bullet assumes its unit is exported per this contract.
...
- [ ] **Move focus INTO the panel on open (round-2 Codex B2 / Claude M2 — the focus-restoration path was unreachable: the opener keeps focus and `UsageCluster` has no focusable child, so focus never entered the panel and fell to `body` on unmount).** On popover open, focus the panel itself: the `.mj_HeaderMenu` popover container is `role="group"` `tabIndex={-1}` and receives `.focus()` in a `useLayoutEffect` gated on the open flag (standard disclosure "focus moves into the disclosed content" pattern). Now focus is defined and inside the panel.
- [ ] **Track focus ownership for BOTH opener and panel, captured while they exist (round-2 Codex B2).** A `focusWithinRef` boolean is set true by an `onFocusCapture` on a wrapper spanning **the mini trigger + its popover**, and re-evaluated on `onBlur` (`focusWithinRef.current = wrapper.contains(e.relatedTarget)`). This records "focus is on the opener OR in the panel" while both still exist — covering the case where the ResizeObserver expands the header while focus is still on the mini trigger (before the user tabbed in).
- [ ] `useEffect` auto-close: close a popover when its collapse flag turns false **or** (usage) when `limits?.length` becomes 0; deps `[usageCollapsed, titleCollapsed, limits?.length]`. On programmatic close, **if `focusWithinRef.current` was true**, move focus to the now-expanded cluster's first focusable, **falling back to `.mj_BackButton`** (a stable always-present node) when the cluster itself is gone (empty-limits path; round-4 M1) — never `document.body`. Reset `focusWithinRef` on user-directed close (Escape/outside-click) so a later unrelated resize doesn't steal focus.
- [ ] Mini triggers: **disclosure pattern** — `aria-expanded` + `aria-controls="<popover-id>"`, **no `aria-haspopup`** (round-4 M2); popovers `role="group"` + `aria-label` + `tabIndex={-1}` (focus target). **Popover anchoring (round-1/round-2 — both popovers anchor to the header, no local wrapper, resolving the M1/M2/M3 ambiguity once):** BOTH `.mj_TitlePopover` and `.mj_UsagePopover` are children of `.mj_ChatHeader`, which is already `position: relative` via the pre-existing `.mx_RoomHeader` rule (`journal.pcss:64-67`). Title popover: `left/right: var(--cpd-space-2x)` (header-width two-edge clamp, no 390px overflow). Usage popover: `right: var(--cpd-space-2x)` (flush to the header's right edge, under the mini-usage pill's column). **Neither mini trigger gets its own `position: relative` wrapper** — the header is the single containing block for both, so nothing is left to implementer discretion and the round-3 B3 overflow can't recur.

**Acceptance:** collapse flags swap full↔mini; click-toggle popovers open/close via the hook; single-popover invariant; **focus moves into the panel on open**; auto-close on threshold-clear AND empty-limits restores focus to a stable node (never `document.body`), with ownership captured pre-unmount; disclosure ARIA; unknown affix visible in collapsed pill; both popovers header-anchored (no 390px overflow). Spec §3, AC#3/#4.
