...
- [ ] **Focus-containment must be captured BEFORE unmount (round-1 Codex B3 — reading `panelRef.current` in a passive effect is too late: the collapse-flag/limits change unmounts the panel in the same commit, so `panelRef.current` is already `null` and `document.activeElement` has moved to `body`).** Track a `focusWithinRef` boolean updated by the popover's `onFocusCapture={() => (focusWithinRef.current = true)}` and `onBlur={(e) => { if (!panelRef.current?.contains(e.relatedTarget as Node)) focusWithinRef.current = false; }}` — this records containment **while the panel exists**. The auto-close `useEffect` then reads the boolean, not the detached ref.
- [ ] `useEffect` auto-close: close a popover when its collapse flag turns false **or** (usage) when `limits?.length` becomes 0; deps `[usageCollapsed, titleCollapsed, limits?.length]`. On programmatic close, **if `focusWithinRef.current` was true**, move focus to the now-expanded cluster's first focusable, **falling back to `.mj_BackButton`** (a stable always-present node) when the cluster itself is gone (empty-limits path; round-4 M1). Test both the threshold-clear and empty-limits transitions with focus placed inside the panel first (deferred full-app render → T-3.2 tests).
- [ ] Mini triggers: **disclosure pattern** — `aria-expanded` + `aria-controls="<popover-id>"`, **no `aria-haspopup`** (round-4 M2); popovers `role="group"` + `aria-label`. **Popover anchoring (round-1 Codex M2 / Claude M3 — resolve the containing block explicitly, don't leave it to the implementer):** `.mj_TitlePopover` anchors to the **full-width header** — it is a child of `.mj_ChatHeader` (which is already `position: relative` via the pre-existing `.mx_RoomHeader` rule, `journal.pcss:64-67`), NOT of a small mini-title wrapper; the `left/right: var(--cpd-space-2x)` clamp (T-3.3) then resolves against the header width so it can't overflow at 390px. `.mj_UsagePopover` anchors to the **mini-usage `position: relative` wrapper** (fine — it's flush to the header's right edge, `right: 0` keeps it on-screen). So: title popover = header-relative; usage popover = local-wrapper-relative. The mini-title itself does NOT get a `position: relative` wrapper (that would make it the containing block and reproduce the round-3 B3 overflow).

**Acceptance:** collapse flags swap full↔mini; click-toggle popovers open/close via the hook; single-popover invariant; auto-close on threshold-clear AND empty-limits with focus captured pre-unmount + safe restoration; disclosure ARIA (no haspopup mismatch); unknown affix visible in collapsed pill; title popover anchored to header (no 390px overflow). Spec §3, AC#3/#4.
...
- [ ] **#500 mini + popovers:** `.mj_HeaderMiniUsage` / `.mj_HeaderMiniTitle` (pill-sized disclosure buttons, `:focus-visible` outline `2px var(--cpd-color-icon-accent-primary)`, worst-metric dot colored by level, unknown affix style). **Anchoring (round-1 M2/M3, per T-2.3):** `.mj_TitlePopover` is a child of `.mj_ChatHeader` (already `position: relative` via `.mx_RoomHeader`, `journal.pcss:64-67`) → `{ left: var(--cpd-space-2x); right: var(--cpd-space-2x); width: auto; max-width: none; top: calc(100% + var(--cpd-space-2x)); }` (header-width two-edge clamp, no 390px overflow). `.mj_UsagePopover` anchors to its mini-usage `position: relative` wrapper → `{ right: 0; top: calc(100% + var(--cpd-space-2x)); }` (flush right, stays on-screen via inherited `width: min(280px, calc(100vw - 32px))`). The mini-title gets **no** local `position: relative` wrapper (that would reintroduce the B3 overflow). Both inherit the `.mj_HeaderMenu` card.
...
**Files:** build the harness in a **scratch dir OUTSIDE the repo/deploy root** — e.g. `/tmp/header-gate/` (round-1 Claude M1: `docs/design/` holds only the static `matron-redesign-v3.dc.html` import bundle, NOT a component harness; the v3 reskin harness was itself a scratch-dir page, deliberately uncommitted because a harness under the deploy/`webapp` root gets shipped publicly or erased by the build). Do NOT check the harness in. Artifacts named `header-<state>-<theme>[-<width>].png`.
- [ ] **Harness construction steps (concrete — round-1 M1):** (1) `corepack pnpm build` in the worktree to produce the compiled CSS; (2) in `/tmp/header-gate/index.html`, `<link>` the built `journal`/`shell` CSS + set the theme class (`.cpd-theme-light`/`-dark`) on a wrapper; (3) mount a static DOM snapshot of the header markup (copy the rendered `.mx_RoomView_body > header` structure with representative fixtures — model name, email, N limits incl. an unknown, running+finished subagent chips) OR render the real components via a tiny bundled entry; (4) wrap in a container whose `width` is set inline per cell; (5) drive captures with the same headless-Chrome tooling the v3 reskin gate used (chrome-devtools/Playwright screenshot).
...
### T-4.2: Rebase, lint, full suite, Codex review (in this order)

**Rebase precedes the final review (round-1 Codex M1) — else a conflict resolution / imported overlapping header change from a sibling window lands AFTER review and escapes it.**

- [ ] **Rebase `vps-header` on `origin/main` FIRST** (`git -C /opt/matron/web-wt-header fetch origin && git -C /opt/matron/web-wt-header rebase origin/main`); the sibling header/UI windows may have merged. Resolve any header-region conflicts (`components.tsx`/`journal.pcss`); if `open-loops.json`-style shared files conflict, take main + re-apply. Re-verify scope.
- [ ] `corepack pnpm lint` (types + prettier) green **post-rebase**.
- [ ] `corepack pnpm test` (jest) green **post-rebase** — including the new `usage-cluster-test.ts`, `adaptive-header-test.ts` (threshold + full-app mount/remount lifecycle + focus restoration, per T-3.2), and extended `subchat-strip-test.ts`.
- [ ] Codex adversarial review on the **post-rebase** branch diff (`codex_adversarial_exec.sh --kind diff` / `/codex-review`); residuals either fixed or filed as follow-up loops. (Reviewing the post-rebase surface is the point — the final diff is what ships.)

**Acceptance:** branch rebased on latest main FIRST; lint + test green post-rebase; Codex review of the post-rebase diff clean or residuals owned. Spec AC#6/#7.
