...
**Test-seam / export contract (round-2 Codex M1).** Tests that render/invoke a unit directly require that unit to be importable. `components.tsx` today exports only `MatronApp`. Therefore: `normalizePercent`/`worstLimit` (`status.ts`) get named `export`s (already stated); and `useAdaptiveHeader`, `UsageCluster`, `SubagentStrip`, **and `HeaderShell`** get **named `export`s from `components.tsx`** so their `*-test.ts` files AND the T-4.2 visual-gate harness can import them (round-3 Codex B2 — the harness mounts `HeaderShell`, so it must be exported; exporting a symbol is not a "new source file" and doesn't fork Dan's layout — it's additive). Only `useDismissablePopover` stays module-private (exercised through the exported components / full-app render). Each unit's test bullet + the harness assume their unit is exported per this contract.
...
- [ ] **Focus model — FROZEN after 4 review passes on this surface (round-1 B3, round-2 Codex B2/Claude M2, round-3 Codex Major3/Claude B1). Minimal correct design, no spanning wrapper, single restore target:**
  ...
  - **On programmatic auto-close** (collapse flag clears, or usage `limits?.length` → 0; deps `[usageCollapsed, titleCollapsed, limits?.length]`): if `focusHeldRef.current` was true, restore focus to **`.mj_BackButton`** — a single stable, always-present, always-focusable node (do NOT hunt for "the expanded cluster's first focusable," which doesn't exist — the inline clusters render only spans/progressbars/heading; round-3 Claude B1). Never `document.body`. Reset `focusHeldRef` on user-directed close (Escape/outside-click) so a later unrelated resize doesn't steal focus.
