...
- [ ] **Restore is symmetric to the forward swap (round-3 B1 — a restore must NOT `mv bak webapp` while `webapp/` exists, same nesting trap):** every restore path = quarantine the current `webapp/` (assert `webapp.failed.$TS` absent → `mv webapp webapp.failed.$TS`, guarded) → assert `webapp` absent → `mv <bak> webapp` (guarded) → health-check the restored release. Recovery precedes any pruning.
...
- [ ] **Durable post-deploy record (round-3 M3 / final-review M1 — P34 observability):** append a structured event to `docs/superpowers/evidence/2026-07-24-reskin/deploy-log.jsonl` — `{sha, ts, backup_path, release_path, live_asset_check: pass|fail, restore_fired: bool, restored_health, final_status: deployed|rolled-back}` — so "deployed" vs "rolled back" is distinguishable after the shell exits / on later audit. **This append happens AFTER the swap, so it needs its own commit + push** (the T-5.3 evidence commit predates the swap and can't contain it): `git add docs/superpowers/evidence/2026-07-24-reskin/deploy-log.jsonl && git commit -m "chore: reskin deploy-log <status>" && git push` — retryable, with a pending-state note if the push fails (the record is on-disk regardless). Then close #497 (retryable bookkeeping — the durable record + filed follow-up loops mean a failed close is resumable, not a lost state).
