...
- [ ] **Durable post-deploy record (round-3 M3 — P34 observability):** append a structured event to `docs/superpowers/evidence/2026-07-24-reskin/deploy-log.jsonl` — `{sha, ts, backup_path, release_path, live_asset_check: pass|fail, restore_fired: bool, restored_health, final_status: deployed|rolled-back}` — so "deployed" vs "rolled back" is distinguishable after the shell exits / on later audit (committed with the evidence). Then close #497 (retryable bookkeeping — the durable record + filed follow-up loops mean a failed close is resumable, not a lost state).
