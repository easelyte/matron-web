...
- [ ] Behavioral gate (logged in, on staging): copy-button copies; relocated timestamp renders exactly once (cross-checks T-2.5); the avatar header renders. **The staging client proxies to the REAL journal backend (round-3 M1 — P19a), so isolate the test:** use a dedicated throwaway test conversation and a predetermined **read-only** prompt (e.g. `!echo` / a trivial no-side-effect message that elicits a short streamed reply + at most a read-only tool like a file `Read`); explicitly NO mutating/egress tool calls — the pre-sign-off gate must not change production state. **Enforcement, not just intent (final-review r3 B1 — a natural-language "read-only" instruction is not a boundary if any tool auto-approves):** prefer **captured fixtures / a mock journal endpoint** for the streaming + ToolStream render checks so the pre-sign-off gate CANNOT reach the production backend at all; if a live interaction is used, point staging at a disposable/scratch journal instance (or a bridge session with a server-side read-only tool allowlist), never the operator's live conversations. Default to fixtures.
...
- [ ] **Guards:** crash-recovery preflight (if `webapp/` absent at start, restore newest bak **directory** by mtime — `ls -1dt webapp.bak.*/ | head -1`, the `-d` is load-bearing, else `ls` lists dir *contents* not the dirs, round-3 B1; NOT lexicographic sort — the live dir has a `webapp.bak.deploy-*` that sorts last but is older). **Pruning order (final-review r3 B2 — never prune before recovery):** in the crash-recovery preflight, SELECT + reserve the recovery candidate and restore it FIRST; prune only NON-selected pre-existing baks AFTER a healthy restore. Do not delete any bak before recovery completes.); `flock` deploy lock; same-FS assert (`stat -f` staging vs live parent); R102 confirmation token gates the whole swap.
