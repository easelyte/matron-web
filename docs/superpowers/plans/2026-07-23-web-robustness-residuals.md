...
- [ ] Extract the dark literal from `theme.ts` source (`readFileSync` of `src/journal/theme.ts`, regex the `resolved === "dark" ? "(#[0-9a-fA-F]{3,8})"` on line 59) and from the `index.html` bootstrap (reuse `bootstrapScript()` + regex the same ternary).
- [ ] Assert all three are equal, case-insensitively (`.toLowerCase()`), and that they equal `#16191d` (a redundant literal pin so a coordinated-but-wrong 3-way edit still fails).
- **Acceptance:** `pnpm test` green; the new test FAILS if any one of the three literals is edited in isolation (prove in dev by temporarily desyncing one, then revert — do NOT commit the desync); add an explicit unit assertion that a dark-block *without* the token throws/fails even when a same-named declaration exists elsewhere (round-2 Codex M3). No production code changed.
- **Principle:** this delivers *drift detection*, not P2 derivation (spec FIX 1 "What this delivers") — the test comment must say "parity guard", not "single source".
...
>
> **R100/R101/R102-compliant lane (round-2 Codex B1, round-3 Codex B3):** loop-store mutation is canonical-state mutation — it must NOT be run inline from main chat. **Route Phase 4 through the `/close` end-of-session flow** (the sanctioned lane that reconciles the loop store and commits it as a `chore(loops)` change through review, matching the repo's `chore(loops): ...` history and CLAUDE.md "Mutations land via `chore(loops): ...` PR commits"), OR an equivalent **Agent-tool spawn with `isolation: worktree`** (`procedure_subagent_followup_filing_via_agent_tool_spawn`). **R102 is a BLOCK rule and its confirmation-token requirement is UNCONDITIONAL for these state-changing mutations (round-3 Codex B3) — it is an explicit precondition, not "if it applies":** the `add_loop`/`close_loop_by_id` mutations proceed only through a lane that satisfies the R102 confirmation-token gate (the `/close` lane provides it by design). Runs from the son-of-anton workspace root **after the matron-web PR merges** (per `procedure_pr_close_keyword_does_not_sync_loop_store`, `procedure_cross_repo_slim_chain_workdir_flag`, `procedure_canonical_store_mutation_from_worktree`). Verify via `load_active_loops()` from the son-of-anton root.
