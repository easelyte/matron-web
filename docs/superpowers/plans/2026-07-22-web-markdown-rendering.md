...
- [ ] `MARKDOWN_MAX = 200_000` constant. Grammars imported individually from `highlight.js/lib/languages/*`
  ...
  **`aliases` option** — `ALIASES = { typescript: ["ts", "tsx"], javascript: ["js", "jsx"], bash: ["sh"],
  ...
  aliases: ALIASES }` — **`detect` DROPPED as a DoS mitigation** during execution (Phase-2 review: `detect: true`
  autodetection on a 20k untrusted untyped fence measured 6.9s main-thread; see spec §Dependencies
  "Implementation revision"). Labeled fences highlight via their language class; untyped fences render
  unhighlighted. (no `ignoreMissing`).
- [ ] `MarkdownBody({ text, streaming = false, label })`, wrapped in `React.memo` on `(text, streaming, label)`:
  - `remarkPlugins={[remarkGfm]}`; `rehypePlugins` = `[[rehypeHighlight, { languages: CURATED, aliases: ALIASES }]]`
    **only when `!streaming`** (omit during streaming — the per-token cost bound; `detect` dropped — DoS). **No
