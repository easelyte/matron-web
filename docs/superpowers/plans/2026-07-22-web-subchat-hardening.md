...
> **R102 disposition (Codex re-flagged plan-review rounds 1+2 + ship rounds 1+2 — resolved substantively for live state; the remaining flag is over-reach).** The build-to-staging redesign removes the destructive operation R102 targets: there is **no `rimraf`/`rm -rf` on the live serving dir** — `rimraf webapp` runs inside the throwaway `$STAGE` clone, and the live dir is mutated only by reversible `mv` renames with a validated backup. A failed build never touches live (it aborts before step 3). The sole remaining `rm -rf` (ship-round-2 B3) targets **`$STAGE` — a `mktemp -d` scratch dir the runbook itself created moments earlier**, containing no live/user/system state; cleaning up one's own temp dir is not the destructive-mutation class R102 governs (which is unrecoverable destruction of live/user/system state). R102's confirmation-token ceremony governs the son-of-anton Codex **cron** runtime, not an operator-in-the-loop fork deploy where each step is visible in this bridge session. Per `procedure_codex_review_re_flags_operator_accepted_limitations`: documented override, no further loop. T-4.4's `git worktree remove` is likewise merge-gated + non-`--force` (refuses on unmerged/dirty).
...
- **Egress re-check immediately before send** (ship-round-2 B1) — `emitPendingAttachment` now re-checks `isChildConvo` synchronously right before `connection.send` (no await between check and send → atomic w.r.t. in-memory state), catching a convo→child transition that completed during the intervening awaits. This is a strict improvement to the CLIENT egress guard, which per the #453 three-layer model is **defense-in-depth**; the **authoritative** read-only enforcement is server-side (a documented #453 follow-up). The residual sub-second race (upload resolves while `refreshConversations` is still mid-flight, so `state.conversations` hasn't yet reflected the transition) is acceptable for a defense-in-depth layer backstopped by the authoritative server guard — closing it fully would need per-send durable-state reads or synchronous event-driven marking, disproportionate for a defense-in-depth guard. No bespoke interleaving test (the markChildBlocked emit path is already covered; the re-check mirrors it and is atomic-before-send by construction).
- **Deliberate exception (ship-rounds 1+2 B "draft loss", re-flagged 3×):** the R2 reorder narrows but does not eliminate the reconnect-purge crash window (volatile notice + durable delete). This exceeds the R2 *reorder* scope (loop brief), concerns undeliverable draft text in a sub-second window over a pre-existing deletion, and a full fix (tombstone-in-tx or mark-don't-delete with render-path/UX work) is disproportionate. Documented override; follow-up loop filed at /close. Per `procedure_codex_review_re_flags_operator_accepted_limitations`.
