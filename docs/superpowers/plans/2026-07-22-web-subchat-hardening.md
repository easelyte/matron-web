...
Expected: **green now** (both effects already occur; the reorder is state-neutral). This confirms the regression-guard baseline before the code change. If modeling a brand-new harness, expect an initial red only until the harness *wiring* is correct — that's a scaffolding error, not the fix's red state.
...
- Test: new `test/unit-tests/journal/upload-child-abort-test.ts`. **New harness — no existing file mirrors this** (see Step 1 note).
...
- [ ] **Step 1: Write the failing tests** — NEW HARNESS, non-trivial scaffolding (Claude round-1 major 2)

**Do NOT expect to "mirror upload-timeout-test.ts"** — that file only unit-tests the pure `uploadTimeoutMsFor(bytes)` function; it constructs no client, no `AbortController`, no fetch mock. The nearest structural precedent is `readonly-egress-test.ts`'s `FakeDatabase` + the `internals(client)` cast, but that fake implements only `outbox/addToOutbox/deleteOutboxRows/events/cursor` — it lacks `applyJournal`, `conversations()`, and `reconcileOwnMessage`, all of which the `handleJournal` entry point (the driver needed to reach `refreshConversations()` → `abortUploadsForChildConvos()`) calls. **Budget this as new-harness work.** The fake/stub shape required:
- (a) a controllable `api.uploadMedia` stub that returns a promise which does NOT resolve until the test allows it (so the upload is genuinely in-flight when the transition fires) — race it against the passed `signal` so `abort()` rejects it, mirroring `uploadPendingAttachment`'s own `Promise.race(..., abortPromise(signal))` shape.
- (b) a `file.arrayBuffer()` that resolves fast (so control reaches `uploadMedia`) — the hang is in `uploadMedia`, not `arrayBuffer`.
- (c) a `FakeDatabase` extended from the `readonly-egress-test.ts` one with `applyJournal(event)` (mutating a convo's `parent_convo_id` from the `convo_meta` payload, mirroring `database.ts:255-261`), `conversations()` (returning current rows so `refreshConversations` sees the new parent), and a no-op `reconcileOwnMessage`.
- (d) driving `handleJournal(convoMetaEvent)` via the `internals(client)` cast with a synthetic `convo_meta` event that sets `parent_convo_id` for the in-flight upload's convo.

Assertions:
```ts
// in-flight upload on a top-level convo; drive handleJournal(convo_meta sets parent) →
//   assert the captured controller.signal.aborted === true, AND after the upload unwinds through
//   the finally, uploadConvos has no stale entry for that localId.
...
- **Verify the merged SHA actually contains all three diffs** before any deploy: `git -C /opt/matron/web-journal show <merged-sha> --stat` (or `git log origin/main -- src/journal/database.ts src/journal/client.ts`) confirms edits to `database.ts` (R1) + `client.ts` (R2, R3). A merge that doesn't touch both files is a red flag — halt, do not deploy.

- [ ] Rebase `feat/subchat-hardening` onto current `origin/main`; ship via `/ship-slim`; confirm PR merged.
- [ ] Fetch `origin/main`; record merged SHA; verify it touches `database.ts` + `client.ts`.

### T-4.3: deploy the merged fork-main to live :8443, operator-verify

**Files:** none (deploy).

**Deploy runbook (atomic, per `CLAUDE.local.md` journal-web runbook — NOT the Matrix-era Docker path in the `matron-web-deploy` memory). Absolute paths throughout; each destructive step is target-validated (Codex B3 — no blind `rm -rf`):**
```bash
WEB=/opt/matron/web-journal
TS=<UTC timestamp, e.g. 20260722T140000Z>
# 1. PULL THE MERGED FIX INTO THE LIVE CHECKOUT FIRST (Claude M1 / Codex B1 — the live checkout
#    sits on pre-fix main until this runs; skipping it silently deploys stale code):
git -C "$WEB" fetch origin
git -C "$WEB" checkout main
git -C "$WEB" pull --ff-only origin main
# 2. CONFIRM the checkout now contains the fix commits before building (guard against building stale):
git -C "$WEB" log --oneline -- src/journal/database.ts src/journal/client.ts | head   # must show the merge
# 3. Atomic backup of the current live bundle (validated non-empty), then build in place:
...
**Acceptance:**
- The live checkout's HEAD contains the merged fix (step-2 log shows the merge) **before** the build runs.
...
- **Deliberate exceptions:**
  ...
  - T-3.1/T-3.2 tests are NEW harness work (no existing file mirrors the in-flight-upload/abort race) — scoped explicitly in T-3.1 Step 1, budgeted accordingly.
